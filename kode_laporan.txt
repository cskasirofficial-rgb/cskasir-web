...
export default function KasirPage() {
  ...
  const [showEditPrice, setShowEditPrice] = useState<CartItem | null>(null);
    ...
    try {
      if (!editMode) {
        ...
        const payload = {
          ...
          totalBelanjaMurni,
          items: cart.reduce((acc, c, i) => ({ 
            ...acc, 
            [i]: { produkId: c.id, nama_produk: c.nama, harga_jual: c.hargaJual, qty: c.qty, modal: c.hargaModal, isCustomPrice: c.isCustomPrice } 
          }), {})
        };
        ...
        cart.forEach(c => {
          ...
          if (p) updates[`stok/${profile?.groupId}/${c.id}/total_stok`] = (p.total_stok || 0) - c.qty;
        });
        ...
        if (itemsLama) {
          Object.values(itemsLama).forEach((il: any) => {
            ...
            if (p) stokUpdates[`stok/${profile?.groupId}/${il.produkId}/total_stok`] = (stokUpdates[`stok/${profile?.groupId}/${il.produkId}/total_stok`] || p.total_stok || 0) + (il.qty || 0);
          });
        }
          ...
          <div className="flex-1 p-6 flex flex-col border-r border-slate-800">
            {editMode && (
              <div className="bg-orange-500/20 border border-orange-500/50 p-4 rounded-xl mb-4 flex justify-between items-center">
                <p className="text-orange-400 font-bold">⚠️ MODE EDIT KOREKSI AKTIF (Trx: #{String(editMode.trxId).slice(-5)})</p>
        ...
        <div className="fixed inset-0 bg-slate-950/80 z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-sm p-6 shadow-2xl">
            ...
            <input 
              ...
              className="w-full bg-slate-800 border-none rounded-xl p-4 text-yellow-400 text-xl font-bold mb-6" 
            />
      ...
      {showSuccess && (
        <div className="fixed inset-0 bg-slate-950/80 z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-emerald-500/30 rounded-3xl w-full max-w-sm p-8 shadow-2xl text-center">
            ...
            <p className="text-slate-400 font-mono mb-6">Trx ID: #{String(showSuccess.trxId).slice(-6)}</p>
